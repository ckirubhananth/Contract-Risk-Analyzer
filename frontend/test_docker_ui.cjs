...
const screenshotPath = path.join('C:', 'Users', 'Kirubhananth', '.gemini', 'antigravity', 'brain', '229bc39a-f5fd-4403-b289-9fc5661d868c', 'belmont_docker_dashboard.png');
const contractPath = path.join(__dirname, '..', 'Contract', 'Belmont.pdf');

(async () => {
  console.log('Launching headless browser for Docker test...');
  ...
  console.log('Navigating to http://localhost (Docker Nginx)...');
  ...
  console.log(`Uploading file to Docker: ${contractPath}...`);
  await fileInput.uploadFile(contractPath);
  
  console.log('Waiting for analysis to complete (Docker FastAPI)...');
  try {
    // Wait for the dashboard grid to appear (max 300 seconds)
    await page.waitForSelector('.dashboard-grid', { timeout: 300000 });
    console.log('Docker Dashboard rendered successfully!');
    ...
    console.log('--- DOCKER UI RENDER VERIFICATION ---');
